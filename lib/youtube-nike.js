...
const NIKE_CHANNEL_ID = "UCmj74ckmwbAjYFVHfLUIZjw";
const BASE_URL = "https://www.googleapis.com/youtube/v3";

export async function getNikeVideos(maxResults = 3) {
  if (!API_KEY || API_KEY === "your_api_key_here") {
    console.warn("YouTube API Key is missing or invalid on server.");
    return [];
  }

  console.log("Fetching Nike Oladeru's YouTube videos from server...");

  try {
    const url = `${BASE_URL}/search?key=${API_KEY}&channelId=${NIKE_CHANNEL_ID}&part=snippet,id&order=date&maxResults=50&type=video`;
    ...
    console.log(`Server-side successfully fetched ${data.items?.length || 0} items from Nike's channel.`);
    ...
    console.log(`Filtered down to ${longFormVideos.length} long-form videos for Nike.`);

    return longFormVideos.map((item) => {
      const rawTitle = decodeHtmlEntities(item.snippet.title);
      // Nike's titles often use " - " as a separator, e.g. "Song Name - Nike Oladeru"
      // Split on " - " and use the first part as the clean title
      ...
      return {
        ...
        series: extractSeries(rawTitle),
        speaker: "Pastor Nike Oladeru"
      };
    });
  } catch (error) {
    console.error("Server-side error fetching Nike's YouTube videos:", error);
    return [];
  }
}
...
function extractSeries(title) {
  // First try brackets or parentheses
  const match = title.match(/\[(.*?)\]/) || title.match(/\((.*?)\)/);
  if (match) {
    return match[1].replace(/[\/|]+/g, " ").replace(/\s+/g, " ").trim() || "Sermon";
  }
  // If title has " - " separator, use the part after the last dash as the series
  const parts = title.split(/\s*[-–—]+\s*/);
  if (parts.length > 1) {
    return parts[parts.length - 1].trim() || "Sermon";
  }
  return "Sermon";
}
